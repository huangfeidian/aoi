...
{
public:
	aoi_manager(aoi_interface* aoi_impl, std::uint32_t max_entity_size, std::uint16_t max_aoi_radius, pos_t min, pos_t max);
	~aoi_manager();
...
public:
	...
	std::vector<guid_t> entity_in_circle(pos_t center, std::uint16_t radius);
	// 获取平面区域内圆柱形区域内的entity列表
	std::vector<guid_t> entity_in_cylinder(pos_t center, std::uint16_t radius, std::uint16_t height);
	// 获取 平面内矩形区域内的entity列表
	std::vector<guid_t> entity_in_rectangle(pos_t center, std::uint16_t x_width, std::uint16_t z_width);
	// 获取平面内正方形区域内的entity 列表
	std::vector<guid_t> entity_in_square(pos_t center, std::uint16_t width);
	// 获取长方体区域内的entity列表
	std::vector<guid_t> entity_in_cuboid(pos_t center, std::uint16_t x_width, std::uint16_t z_width, std::uint16_t y_height);
	// 获取立方体区域内的entity列表
	std::vector<guid_t> entity_in_cube(pos_t center, std::uint16_t width);
	// 获取2d 扇形区域内的entity 列表 yaw为扇形中心朝向的弧度， yaw range为左右两侧的弧度偏移
	std::vector<guid_t> entity_in_fan(pos_t center, std::uint16_t radius, float yaw, float yaw_range);
private:
