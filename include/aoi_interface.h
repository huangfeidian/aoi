...
namespace spiritsaway::aoi
{
	...
	public:
		...
		virtual std::vector<aoi_entity*> entity_in_circle(pos_t center, pos_unit_t radius) const = 0;
		// 获取平面区域内圆柱形区域内的entity列表
		virtual std::vector<aoi_entity*> entity_in_cylinder(pos_t center, pos_unit_t radius, pos_unit_t height) const = 0;

		// 获取 平面内矩形区域内的entity列表
		virtual std::vector<aoi_entity*> entity_in_rectangle(pos_t center, pos_unit_t x_width, pos_unit_t z_width) const = 0;
		// 获取长方体区域内的entity列表
		virtual std::vector<aoi_entity*> entity_in_cuboid(pos_t center, pos_unit_t x_width, pos_unit_t z_width, pos_unit_t y_height) const = 0;
		...
		virtual void update_all(const std::vector<aoi_entity*>& all_entities) = 0;
